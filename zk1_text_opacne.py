def list_reverse(list1):
    """The function takes in a list and returns the same list in reversed order.
        ...
        Returns:
                    new_list(list1): Output list with elements in reversed order in comparison with the input list.
    """
...
try:
    ...
    text = input("File text.txt was not found in the same folder from which you are running this program, or file loading failed . You can enter text manually:")
...
text_list = list_reverse(text_list)
text_list = interpunction(text_list)
if capitals_yes == "1":
    text_list = capital_letter(text_list)
text_list_joined = ' '.join(text_list)
print(text_list_joined)
write_yes = input("Do you wish to save generated reversed text to new text file? Enter 1 for YES.")
if write_yes == "1":
    write_text("reversed.txt", text_list_joined)